...
于是我们可以看出：对于某个$t+\Delta$位置向量的某一个维度$2k$或者$2k+1$而言，可以表示为位置$t$和位置$\Delta$的在$2k$或者$2k+1$维度的位置向量线性组合。
...
> 简短描述attention的过程：把Values拆分成key-value形式，并


将 Query(Q) 和 key-value pairs（把 Values 拆分成了键值对的形式） 映射到输出上，其中 query、每个 key、每个 value 都是向量，输出是 V 中所有 values 的加权，其中权重是由 Query 和每个 key 计算出来的
