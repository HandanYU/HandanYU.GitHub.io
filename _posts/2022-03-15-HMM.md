...
- 有一种错误的想法，对每个word，选择一个tag maximises $$P(w_i|t_i)P(t_i|t_{i-1})$$
...
- 对于第二个单词**fish**
  ...
  - 对于$$s(N|fish)=P(fish|N)\times P(N|they_{tag})\times s(they_{tag}|they)$$
    ...
    - If $$they_{tag}=N$$: $$s(N|fish)=P(fish|N)\times P(N|N)\times s(N|they)=0.3\times 0.8\times 0.42=0.1008$$
  ...
  - 对于$$s(V|fish)=P(fish|V)\times P(V|they_{tag})\times s(they_{tag}|they)$$
    ...
    - If $$they_{tag}=N$$: $$s(V|fish)=P(fish|V)\times P(V|N)\times s(N|they)=0.6\times 0.2\times 0.42=0.0504$$
...
- models describe $$P(y|X)$$ directly
